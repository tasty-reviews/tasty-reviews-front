  ...
  export default {
    data() {
      return {
        ...
        reviewId: null // 리뷰 ID 추가
      };
    },
    async created() {
      const storeId = this.$route.params.storeId;
      const reviewId = this.$route.params.reviewId; // 리뷰 ID 추가
      this.reviewId = reviewId;
      try {
        const storeResponse = await axios.get(`http://localhost:8080/place/${storeId}`);
        this.store = storeResponse.data;

        const reviewResponse = await axios.get(`http://localhost:8080/reviews/${reviewId}`);
        const reviewData = reviewResponse.data;
        this.selectedRating = reviewData.rating;
        this.review.comment = reviewData.comment;
        if (reviewData.imageUrl) {
          this.review.image = reviewData.imageUrl;
        }
      } catch (error) {
        console.error('가게 또는 리뷰 정보를 가져오는 도중 오류 발생:', error);
      }
    },
    methods: {
      goBack() {
      ...
      async submitReview() {
        ...
        try {
          const response = await axios.put(`http://localhost:8080/api/reviews/${this.reviewId}`, formData, {
            headers: {
          ...
          this.goBack(); // 리뷰 수정 후 이전 페이지로 이동
        } catch (error) {
